...
export default {
		...
		async QueryIssuers({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryIssuers({...query, 'pagination.key':(<any> value).pagination.next_key})).data
	...
	gas: "200000" }, memo})
				...
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgBurnToken:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgBurnToken:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		async sendMsgIssueUserCredential({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgIssueUserCredential(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgIssueUserCredential:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgIssueUserCredential:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
	...
	gas: "200000" }, memo})
		...
		async sendMsgPauseToken({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgPauseToken(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgPauseToken:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgPauseToken:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
	...
	gas: "200000" }, memo})
		...
		async MsgBurnToken({ rootGetters }, { value }) {
			try {
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgBurnToken:Create Could not create message: ' + e.message)
				}
			}
		},
		async MsgIssueUserCredential({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgIssueUserCredential(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgIssueUserCredential:Create Could not create message: ' + e.message)
				}
			}
		},
		...
		async MsgPauseToken({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgPauseToken(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgPauseToken:Create Could not create message: ' + e.message)
				}
			}
		},
