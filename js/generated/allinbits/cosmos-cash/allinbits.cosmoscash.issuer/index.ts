...
export default {
	...
	gas: "200000" }, memo})
				...
				if (e == MissingWalletError) {
					throw new SpVuexError('TxClient:MsgPauseToken:Init', 'Could not initialize signing client. Wallet is required.')
				}else{
					throw new SpVuexError('TxClient:MsgPauseToken:Send', 'Could not broadcast Tx: '+ e.message)
				}
			}
		},
	...
	gas: "200000" }, memo})
				...
				if (e == MissingWalletError) {
					throw new SpVuexError('TxClient:MsgIssueUserCredential:Init', 'Could not initialize signing client. Wallet is required.')
				}else{
					throw new SpVuexError('TxClient:MsgIssueUserCredential:Send', 'Could not broadcast Tx: '+ e.message)
				}
			}
		},
		...
		async MsgPauseToken({ rootGetters }, { value }) {
			try {
				...
				if (e == MissingWalletError) {
					...
					throw new SpVuexError('TxClient:MsgPauseToken:Create', 'Could not create message: ' + e.message)
		...
		async MsgIssueUserCredential({ rootGetters }, { value }) {
			try {
				...
				if (e == MissingWalletError) {
					...
					throw new SpVuexError('TxClient:MsgIssueUserCredential:Create', 'Could not create message: ' + e.message)
