...
const baseMsgCreateDidDocument = {
    id: "",
    controllers: "",
    signer: "",
};
...
const baseMsgUpdateDidDocument = { signer: "" };
...
const baseMsgAddController = { id: "", controllerDid: "", signer: "" };
export const MsgAddController = {
    ...
    decode(input, length) {
        const reader = input instanceof Uint8Array ? new Reader(input) : input;
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseMsgAddController };
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1:
...
const baseMsgAddControllerResponse = {};
...
export const MsgDeleteController = {
    ...
    decode(input, length) {
        const reader = input instanceof Uint8Array ? new Reader(input) : input;
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseMsgDeleteController };
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1:
...
const baseMsgDeleteControllerResponse = {};
...
export class MsgClientImpl {
    ...
    AddController(request) {
        ...
        const promise = this.rpc.request("allinbits.cosmoscash.did.Msg", "AddController", data);
        return promise.then((data) => MsgAddControllerResponse.decode(new Reader(data)));
    }
    DeleteController(request) {
        const data = MsgDeleteController.encode(request).finish();
        const promise = this.rpc.request("allinbits.cosmoscash.did.Msg", "DeleteController", data);
        return promise.then((data) => MsgDeleteControllerResponse.decode(new Reader(data)));
    }
}
