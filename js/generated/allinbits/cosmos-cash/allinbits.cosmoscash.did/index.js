...
export default {
    ...
    getters: {
        ...
        getDidDocumentB64: (state) => (params = { params: {} }) => {
            if (!params.query) {
                params.query = null;
            }
            return state.DidDocumentB64[JSON.stringify(params)] ?? {};
        },
    ...
    actions: {
        ...
        async QueryDidDocuments({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.queryDidDocuments({ ...query, 'pagination.key': value.pagination.next_key })).data;
        ...
        async QueryDidDocumentB64({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                const key = params ?? {};
                const queryClient = await initQueryClient(rootGetters);
                let value = (await queryClient.queryDidDocumentB64(key.id_base64)).data;
                commit('QUERY', { query: 'DidDocumentB64', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryDidDocumentB64', payload: { options: { all }, params: { ...key }, query } });
                return getters['getDidDocumentB64']({ params: { ...key }, query }) ?? {};
            }
            catch (e) {
                throw new SpVuexError('QueryClient:QueryDidDocumentB64', 'API Node Unavailable. Could not perform query: ' + e.message);
            }
        },
